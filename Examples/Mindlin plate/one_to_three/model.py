...
def FR_model(x_train, y_train, x_test, y_test, mp1, mp2, mp3, mp4, mp5, mp6, mp7, mp8):
    ...
    output_y = tf.placeholder(tf.float32, shape=[None, 64 * 64 * 3])
    ...
    y_hat = FR_25_alpha(x, is_training, keep_prob)
    y = tf.transpose(tf.reshape(output_y, [-1, 3, 64, 64]), perm=[0, 2, 3, 1])
